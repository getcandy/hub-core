    ...
    <div v-for="group in groups" :key="group.id" class="flex">
      <div class="w-1/2 border-r">
        <div class="flex justify-between py-2 px-6 bg-gray-200">
          ...
          <button v-if="assigned(group.attributes.data).length" class="text-gray-500 text-xs font-medium uppercase hover:text-red-600" @click="removeAll(group.id)">
            Remove all
          </button>
        </div>
        <div class="bg-gray-100  h-full">
          <div v-for="attribute in assigned(group.attributes.data)" :key="attribute.id" class="px-6 bg-white py-2 border-b">
            <div class="flex items-center justify-between">
              {{ getLocaleValue(attribute.name) }}
              <button v-if="!attribute.system" class="bg-gray-200 text-gray-600 text-xs font-medium rounded-lg px-2 py-1 uppercase hover:bg-gray-300" @click="remove(attribute.id)">
                Remove
              </button>
              <span v-else class="inline-flex text-orange-400 mr-6">
      ...
      <div class="w-1/2">
        <div class="flex justify-between py-2 px-6 bg-gray-200">
          ...
          <button v-if="available(group.attributes.data).length" class="text-gray-500 text-xs font-medium uppercase hover:text-green-600 disabled:opacity-75" @click="addAll(group.id)">
            Add all
          </button>
        </div>
        <div class="bg-gray-100 h-full">
          <div v-for="attribute in available(group.attributes.data)" :key="attribute.id" class="px-6 bg-white py-2 border-b">
            <div class="flex items-center justify-between">
              {{ getLocaleValue(attribute.name) }}
              <button class="bg-gray-200 text-gray-600 text-xs font-medium rounded-lg px-2 py-1 uppercase hover:bg-gray-300" @click="add(attribute.id)">
                Add
              </button>
            </div>
          </div>
