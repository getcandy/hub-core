        ...
        <template v-if="loaded">
            <v-card>
                <v-card-title primary-title>
                    <h3>{{ product.attribute_data.name.webstore.en }}</h3>
                </v-card-title>
            <v-tabs
                v-model="tab"
            >
                <v-tab key="details">Product Details</v-tab>
                <v-tab key="media">Media</v-tab>
                <v-tab key="availability">Availability &amp; Pricing</v-tab>
            </v-tabs>

            <v-tabs-items v-model="tab">
                <v-tab-item :transition="false" :reverse-transition="false" key="details">
                    <v-tabs vertical>
                        <v-tab v-for="group in attribute_groups" :key="group.id">
                            {{ group.name.en }}
                        </v-tab>
                        <v-tab-item :transition="false" :reverse-transition="false" v-for="group in attribute_groups" :key="group.id">
                            <product-details  :product="product" :languages="languages" :group="group">
                            </product-details>
                        </v-tab-item>
                    </v-tabs>
                </v-tab-item>
                <v-tab-item :transition="false" :reverse-transition="false" key="media">
                    <media-picker
                        assetable="products"
                        :parent="product">
                    </media-picker>
                </v-tab-item>
                <v-tab-item :transition="false" :reverse-transition="false" key="availability">
                    Availability!
                </v-tab-item>
            </v-tabs-items>
            </v-card>
...
<script>
    import ProductDetails from './edit/ProductDetails.vue';
    import MediaPicker from './Media.vue';
    ...
    export default {
        ...
        methods: {
             ...
             */
            loadLanguages() {
                this.$gc.languages.get().then(response => {
            ...
            loadProduct(id) {
                ...
                this.$gc.products.find(id, {
                    ...
                    this.decorate(response.data.data);
                    this.loaded = true;
