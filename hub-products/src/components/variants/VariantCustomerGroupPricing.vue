<template>
  <div>
    <div class="md:grid md:grid-cols-2 md:gap-6" v-for="price in pricing" :key="price.customer_group_id">
      ...
      <form-field :label="$t('Tax')">
        <select-input v-model="price.tax_id" @input="handleChange">
          <option v-for="tax in taxes" :value="tax.id" :key="tax.id">{{ tax.name }} ({{ tax.percentage }}%)</option>
        </select-input>
      </form-field>
    </div>
  </div>
